...
export const fetchPriceByCurrency = async (
    currency: string
): Promise<number> => {
    try {
        const priceResponse = await suggestedParamsAxios.get(
            `${currency}-USD/buy`
        );
...
export const fetchAllCurrencies = async (): Promise<CryptoPrices> => {
    try {
        const currenciesToWatch = getCurrencyList().map(
            (currency): Promise<number> => fetchPriceByCurrency(currency)
        );
        const returnedFetchArray = await Promise.all(currenciesToWatch);
        const keys = Object.values(CurrencyTypes);
        const values = returnedFetchArray;
        const mergedArray = keys.reduce(
